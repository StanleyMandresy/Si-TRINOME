...
class Type {
    ...
    public function getCategoriebyID($id) {
        ...
        public function addCategorie($nom,$nature) {

        try{
     
                $sql = "INSERT INTO Categorie(nomCategorie,idNature) 
                VALUES ('$nom', '$nature')";
               
               if( !empty($nom) && !empty($nature)){
                $stmt = $this->db->prepare($sql);
                $stmt->execute();
        ...
        public function changeCategorie($idcategorie, $nom, $nature) {
            try {
                $fields = [];
        
                if (!empty($nom)) {
                    $fields[] = "nomCategorie = '" . addslashes($nom) . "'";
                }
                if (!empty($nature)) {
                    $fields[] = "idNature = " . intval($nature);
                }
                ...
                $sql = "UPDATE Categorie SET " . implode(", ", $fields) . " WHERE idCategorie = " . intval($idcategorie);
